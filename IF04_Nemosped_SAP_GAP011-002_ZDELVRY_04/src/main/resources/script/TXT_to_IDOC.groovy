...
def Message processData(Message message) {
    ...
    def len_ZE1EDL11 = [20, 5, 13, 20, 18] as int[];

    def TABNAM = "EDI_DC40";
    def IDOCTYP = "ZDELVRY_04";
    def CIMTYP = "";
    def MESTYP = "ZDELVRY_04";
    def SNDPRT = "LS";
    def RCVPRT = "LS";
    ...
    //     }
        ...
        if (rows[i].startsWith("E1EDL24")) {
            ...
            def segmentToInsert = """<E1EDL24 SEGMENT="1"><POSNR>""" + values[0].replaceFirst ("^0*", "") + "</POSNR><MATNR>" + values[1].replaceFirst ("^0*", "") + "</MATNR><CHARG>" + values[2] + "</CHARG><LFIMG>" +
                                values[3].replaceFirst ("^0*", "") + "</LFIMG><VRMKE>" + values[4].replaceFirst ("^0*", "") + "</VRMKE></E1EDL24>";
        ...
        if (rows[i].startsWith("E1EDL11")) {
            ...
            values = (FixedValues(rows[i].substring("E1EDL11".length()), len_ZE1EDL11));

            def segmentToInsert = """<ZE1EDL11 SEGMENT="1"><DESTHU>""" + values[0].replaceFirst ("^0*", "") + "</DESTHU><ZTYPEHU>" + values[1].replaceFirst ("^0*", "") + "</ZTYPEHU><ZQUANTHU>" + values[2].replaceFirst ("^0*", "") + "</ZQUANTHU><ZSERIALE>" + values[3] + "</ZSERIALE><ZGTIN>" + values[4] + "</ZGTIN></ZE1EDL11>";
    ...
    idoc = """<?xml version="1.0" encoding="UTF-8"?><ZDELVRY_04><IDOC BEGIN="1">""" + idoc + "</IDOC></ZDELVRY_04>";
